...
void SocketTransmitter::send(std::string data) { return this->send((uint8_t *) data.c_str(), data.length()); }

void SocketTransmitter::send(std::vector<uint8_t> data) { return this->send(data.data(), data.size()); }
