...
CC1101::CC1101() : gdo0_pin_(nullptr), frequency_mhz_(868.95f) {}

void CC1101::set_gdo0_pin(InternalGPIOPin *pin) { this->gdo0_pin_ = pin; }

void CC1101::set_frequency(float freq_mhz) { this->frequency_mhz_ = freq_mhz; }
...
void CC1101::setup() {
  ...
  if (version == version_unset || version == version_not_detected) {
    ESP_LOGE(TAG, "CC1101 not detected (PARTNUM=0x%02X VERSION=0x%02X)", partnum, version);
    this->mark_failed();
    return;
  }
...
void CC1101::attach_interrupt_impl(void (*callback)(void *), void *arg) {
  ...
  this->gdo0_pin_->attach_interrupt<InterruptContext>(
      ...
      &interrupt_context_, gpio::INTERRUPT_RISING_EDGE);
}
...
bool CC1101::is_sync_active_() { return this->gdo0_pin_->digital_read(); }
