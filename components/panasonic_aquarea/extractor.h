...
namespace esphome
{
    namespace panasonic_aquarea
    {
        ...
        {
        public:
            static uint8_t read_bits(const std::vector<uint8_t> &data,
                                     size_t byte, uint8_t bit, uint8_t bit_width)
            {
            ...
            static uint32_t read_bytes(const std::vector<uint8_t> &data,
                                       size_t byte, uint8_t num_bytes)
            {
                uint32_t result = 0;
            ...
            // TODO: done inside esphome/helpers.h?
            template <size_t N>
            static std::array<uint8_t, N> read_array(const std::vector<uint8_t> &data,
                                                     size_t byte)
            {
        ...
        {
        public:
            BinaryExtractor(uint8_t byte, uint8_t bit)
                : byte_(byte), bit_(bit) {}
        ...
        {
        public:
            FloatExtractor(size_t byte, uint8_t bit_width,
                           float offset, float multiplier)
                : byte_(byte), bit_width_(bit_width),
        ...
        template <size_t N>
        class StringArrayExtractor : public ExtractorInterface<std::string>
        {
        public:
            StringArrayExtractor(size_t byte, uint8_t bit, uint8_t bit_width,
            ...
            optional<std::string> decode(const std::vector<uint8_t> &data) override
            {
                ...
                {
                    ESP_LOGW(TAG, "invalid index %zu for string extractor (byte %zu bit %u)",
                             index, byte_, bit_);
                    return {};
                }
            ...
            void encode(std::vector<uint8_t> &data, const std::string &value) override
            {
                ...
                if (it == strings_.end())
                {
                    ESP_LOGW(TAG, "string '%s' not found in extractor array (byte %zu bit %u)",
                             value.c_str(), byte_, bit_);
                    return;
                }
        ...
        class StringMapExtractor : public ExtractorInterface<std::string>
        {
        public:
            StringMapExtractor(size_t byte,
                               uint8_t bit,
                               uint8_t bit_width,
            ...
            optional<std::string> decode(const std::vector<uint8_t> &data) override
            {
                ...
                if (it == strings_.end())
                {
                    ESP_LOGW(TAG, "key not found for string extractor (byte %zu, %zu bytes): %s",
                             byte_, KeyLen, format_hex_pretty(key.data(), KeyLen).c_str());
            ...
            void encode(std::vector<uint8_t> &data, const std::string &value) override
            {
                ...
                auto it = std::find_if(strings_.begin(), strings_.end(),
                                       [&value](const std::pair<std::array<uint8_t, KeyLen>, std::string> &pair)
        ...
        template <typename T>
        class LambdaExtractor : public ExtractorInterface<T>
        {
        public:
            using Decoder = std::function<optional<T>(const std::vector<uint8_t> &)>;
            using Encoder = std::function<void(std::vector<uint8_t> &, const T &)>;

            LambdaExtractor(Decoder decoder, Encoder encoder = nullptr)
                : decoder_(decoder), encoder_(encoder) {}
            optional<T> decode(const std::vector<uint8_t> &data) override
            {
        ...
        class ReadOnlyEntity : public panasonic_aquarea::ReadableEntity
        {
        public:
            ...
            void handle_update(const std::vector<uint8_t> &data) override
            {
                ...
                if (!value.has_value() ||
                    publish_dedup_.next(*value) == false)
                {
        ...
        template <typename Derived, typename T>
        class WriteOnlyEntity : public panasonic_aquarea::WritableEntity
        {
        public:
            void set_extractor(ExtractorInterface<T> *extractor)
            {
        ...
        template <typename Derived, typename T>
        class ReadWriteEntity : public ReadOnlyEntity<Derived, T>,
                                public WriteOnlyEntity<Derived, T>
        {
        public:
            void set_extractor(ExtractorInterface<T> *extractor)
            {
